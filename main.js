...
function calcIV(
  ...
  resultIV.innerHTML = `Dose For child <span style = "color:red;">${Math.round(resultIV2 * 100) / 100}</span> mg`;
  ...
  if (state === 3) {
    rateIV.innerHTML = `Rate of infusion is <span style ="color:red;">${Math.round((resultIV2  / minFurtherDilution) * 100) / 100}</span> ml/hr`;
    console.log("here");
    resultIV.innerHTML = `Dose For child <span style = "color:red;">${
      Math.round(resultIV2 * 24 * 100) / 100
    }</span> mcq`;
    takenIV.innerHTML = `volume to be taken from vial is <span style ="color:red;">${
  ...
  if (state === 6) {
    ...
    resultIV.innerHTML = `Dose For child <span style = "color:red;">${Math.round(resultIV2 * 100) / 100}</span> gm`;
    maxIV.innerText = ``;
    minumimIV.innerText = ``;
    ...
    rateIV.innerHTML = `Rate of infusion is <samp style="color:red;">${
      Math.round((resultIV2 *24*60/1000 / minFurtherDilution /24) * 100) / 100
    }</samp> ml/hr`;
    maxIV.innerText = ``;
    resultIV.innerHTML = `Dose For child <span style = "color:red;">${
      Math.round(resultIV2 * 24* 60 / 1000 * 100) / 100
    }</span> mg`;
    takenIV.innerHTML = `volume to be taken from vial is <span style ="color:red;">${
      Math.round(resultIV2 *24* 60 / 1000 * volumeToBeTakenFromVial * 100) / 100
    }</span> ml`;
    minumimIV.innerHTML = `Dilution is <span style="color:red";>${
      Math.round((resultIV2 *24*60/1000 / minFurtherDilution) * 100) / 100
    }</span> ml diluent`;
   }else if (state === 8) {
    rateIV.innerHTML = `Rate of infusion is <samp style="color:red;">${
      Math.round((resultIV2 / minFurtherDilution / 24 ) * 100) / 100
    }</samp> ml/hr`;
    maxIV.innerText = ``;
    resultIV.innerHTML = `Dose For child <span style = "color:red;">${
      Math.round(resultIV2 * 100) / 100
    }</span> mg`;
    takenIV.innerHTML = `volume to be taken from vial is <span style ="color:red;">${
      Math.round(resultIV2 * volumeToBeTakenFromVial * 100) / 100
    }</span> ml`;
    ...
    resultIV.innerHTML="Dose For child 125 mg";
    takenIV.innerHTML="volume to be taken from vial is 1 ml";
    maxperdoseIV.innerHTML="";
    minumimIV.innerHTML="";
    maxIV.innerHTML="";
  }
}
function calcApearIV() {
  ...
  if (drugname.value === "Ceftriaxone") {
    ...
    calcIV(
      ...
      " not taken as push",
      "is",
      "500 mg / 5 ml  ready to use ampoule (fd 50 :100)",
      "NS , D5W ,D10W ",
      "1000mg / day",
      "100 mg ",
      " INR , PT "
...
function calcoralapear() {
  ...
  if (doseNameOral.value === "Metronidazole syp") {
    calcOral(
      125 / 5,
      500,
      resultOral,
  ...
  if (doseNameOral.value === "Baclofen 25 mg tab") {
    calcOral(
      25 / 10,
      "60 mg / day ",
      resultOral,
...
function calcIvMan() {
  ...
  minDilutionOutput.innerText = `Min dilution is ${Math.round(minDilution * 100)/100} ml`;
  volumeToBeTakenOutput.innerText = `volume to be taken from vial is ${Math.round(volumeToBeTaken*100)/100} ml`;
  childDoseOutput.innerText = `Dose  for child is ${Math.round(childDose*100)/100} mg`;
}
...
function calcOralMan() {
  ...
  volumeToBeTakenOutputOral.innerText = `volume to be taken is ${Math.round(volumeToBeTaken*100)/100} ml`;
  childDoseOutputOral.innerText = `dose child is ${Math.round(childDose*100)/100} mg`;
}
